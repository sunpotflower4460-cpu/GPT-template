...
- **`phase-not-bundled` / `no-new-deps` はPRブランチ上でのみ意味を持つ。** ローカルで `main` に直接乗ったまま `npm run guard` を実行すると比較対象がHEAD自身に収束し、これら2チェックは実質何もしない。CI（`guard.yml`）は常にPRブランチ上で走るため問題ないが、ローカル実行だけで「問題なし」と判断しないこと
