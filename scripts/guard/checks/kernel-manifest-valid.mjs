...
export function run({ root }) {
  ...
  if (!health.valid) {
    return {
      ok: false,
      messages: [
        'project-kernel.json が存在しますが不正です（不正なJSON、またはschemaVersion(=== 1)/kind(=== "ai-project-kernel")/paths(非空オブジェクト、値はtrim後に安全な相対パス文字列、NUL文字不可)/capabilities(値がすべてboolean)/modes(配列の場合、全要素がtrim後non-emptyな文字列)/contextRouting(各tierがpathsの既存キーを指す文字列配列)/runtime(存在する場合、値がすべてnon-empty文字列のオブジェクト)/validation(存在する場合、strategiesが配列で各typeがpush/pull_request/workflow_dispatchのいずれか、required(あれば)がboolean、branches(あれば)が文字列配列、checksが配列でnameを持つ)/governance(存在する場合オブジェクトで、maintainerModeがあればSOLO_MAINTAINERまたはMULTI_MAINTAINER)のいずれかを満たしていません）。',
